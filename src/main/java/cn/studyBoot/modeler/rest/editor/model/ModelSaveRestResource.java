package cn.studyBoot.modeler.rest.editor.model;
 ...
 * @RestController
 * 
 * @RequestMapping(value = "/service") public class ModelSaveRestResource
 * implements ModelDataJsonConstants {
 * 
 * protected static final Logger LOGGER =
 * LoggerFactory.getLogger(ModelSaveRestResource.class);
 * 
 ...
 * @RequestMapping(value="/model/{modelId}/save", method = RequestMethod.PUT)
 * 
 * @ResponseStatus(value = HttpStatus.OK) public void saveModel(@PathVariable
 * String modelId, @RequestBody MultiValueMap<String, String> values) { try {
 * 
 * Model model = repositoryService.getModel(modelId);
 * 
 ...
 * TranscoderInput input = new TranscoderInput(svgStream);
 * 
 * PNGTranscoder transcoder = new PNGTranscoder(); // Setup output
 * ByteArrayOutputStream outStream = new ByteArrayOutputStream();
 * TranscoderOutput output = new TranscoderOutput(outStream);
 * 
 * // Do the transformation transcoder.transcode(input, output); final byte[]
 * result = outStream.toByteArray();
 * repositoryService.addModelEditorSourceExtra(model.getId(), result);
 * outStream.close();
 * 
 * } catch (Exception e) { LOGGER.error("Error saving model", e); throw new
 * ActivitiException("Error saving model", e); } } }
 */